...
We can treat the above as a finite state machine, with a state space equal to $\mathcal{S}=\{0, 1\}^L$ and a action space equal to set of buttons $\mathcal{A}=\mathcal{B}$. At each timestamp we are able to choose one action, which will then cause a transition to a different state. As with any finite state machine, we can associate a directed graph with this state machine, whereby the state space the vertices of this graph and the actions for a given state represent the directed edges between these vertices.
